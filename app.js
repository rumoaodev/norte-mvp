...
const stages=[
{title:'Entender sua atividade',desc:'Transformar o que você faz em informações que os órgãos vão pedir, sem começar pelo burocratês.',kind:'inside'},
...
function stageCard(s,i){
 const status=state.stageResults[i]==='done'?'done':i===state.stage?'current':i>state.stage?'locked':'';
 const marker=state.stageResults[i]==='done'?'✓':i+1;
 let actions='';
 if(i===state.stage && state.stageResults[i]!=='done'){
   if(s.kind==='official') actions+=`<a class="official" href="${s.url}" target="_blank" rel="noopener">↗ ${s.label}</a>`;
   else actions+=`<button class="official" data-complete="${i}">Concluir esta preparação</button>`;
   if(s.kind==='official') actions+=`<button class="ghost" data-result="${i}">Já voltei / informar resultado</button>`;
   actions+=`<button class="ghost" data-ai="${i}">✦ Não entendi / Norte IA</button>`;
 }
 return `<article class="stage ${status}"><div class="stage-line"><div class="stage-index">${marker}</div><div class="stage-copy"><div class="eyebrow">${state.stageResults[i]==='done'?'Concluído':i===state.stage?'Agora':i>state.stage?'Depois':'Revisar'}</div><h3>${s.title}</h3><p>${s.desc}</p></div></div><div class="stage-actions">${actions}</div><div id="result-${i}" class="result-panel"><strong>Como essa etapa terminou?</strong><div class="result-options"><button data-stage-result="${i}:done">✓ Concluí</button><button data-stage-result="${i}:pending">⏳ Está em análise</button><button data-stage-result="${i}:error">! Deu erro / pendência</button><button data-stage-result="${i}:confused">? Não entendi</button></div></div></article>`
}
...
function render(){
 ...
 if($('#start'))$('#start').onclick=()=>Object.keys(state.answers).length?nav('route'):openWizard;
...
function summary(){
 ...
 $('#build').onclick=()=>{state.stage=0;state.stageResults={};save();closeWizard();nav('route');toast('Sua rota foi montada.')};
}
